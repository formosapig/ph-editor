...
def _read_bytes(stream: BytesIO, length: int) -> bytes:
    ...
    if len(data) < length:
        raise EOFError(
            f"資料流結束，無法讀取 {length} 位元組。已讀取 {len(data)} 位元組。"
        )
    return data
...
def _format_color_for_json(color_data: dict) -> str:
    if any(
        c is None or c != c
        for c in (color_data["r"], color_data["g"], color_data["b"], color_data["a"])
    ):
        return "NaN"
...
def _format_float_to_value(
    source: float, scale: float = 100, min_val: int = 0, max_val: int = 100
) -> int:
    """
    將浮點數轉換為指定範圍內的整數百分比值。
    ...
    if value < min_val or value > max_val:
        logger.warning(
            f"Source {source} resulted in {value}%, clamped to [{min_val}, {max_val}]."
        )
    return max(min_val, min(max_val, value))


def _parse_value_to_float(
    value: int, scale: float = 100, min_val: int = 0, max_val: int = 100
) -> float:
    """
    將整數百分比轉回浮點數，並檢查範圍。
    ...
    """
    if value < min_val or value > max_val:
        logger.warning(f"Value {value}% out of range [{min_val}, {max_val}].")
    return value / scale


# 組合函式, 方便一次處理完...
def _read_and_format_to_value(
    stream: BytesIO, scale: float = 100, min_val: int = 0, max_val: int = 100
) -> int:
