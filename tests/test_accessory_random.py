...
from tests.utils import compare_dicts, _random_color_str # _random_hex_string is not directly used for data generation but for internal checks
...
# 導入配飾數據
from game_data.accessory_data import ACCESSORY_ITEMS, C0, C1, C2, C3, ACCESSORY_SLOT, ACCESSORY_TYPE
...
def format_accessory_for_test() -> dict:
    ...
    if color_flag == C3:
        ...
        formatted_data.update({
            ...
            'main_strength': random.randint(0, 100), # _parse_and_pack_float uses default scale=100
            'main_texture': random.randint(0, 100),  # _parse_and_pack_float uses default scale=100
        ...
        formatted_data['!color_mark'] = "00 00 00 00" # Dummy value indicating no extra color data

    return formatted_data


@pytest.mark.parametrize("_", range(300)) # Reduced iterations for faster testing; consider 300+ for thoroughness
def test_random_accessory_roundtrip(_):
    ...
    for i in range(1, 11):
        ...
        orig_type_name = ACCESSORY_TYPE[0]['name']['zh'] if orig_item.get('type') == -1 else (next((t['name']['zh'] for t in ACCESSORY_TYPE if t['id'] == orig_item.get('type')), 'Unknown Type'))
        parsed_type_name = ACCESSORY_TYPE[0]['name']['zh'] if parsed_item.get('type') == -1 else (next((t['name']['zh'] for t in ACCESSORY_TYPE if t['id'] == parsed_item.get('type')), 'Unknown Type'))
        
        print(f"    Original Type: {orig_type_name} ({orig_item.get('type')}), ID: {orig_item.get('id')}, Slot: {orig_item.get('slot')}")
        print(f"    Parsed Type: {parsed_type_name} ({parsed_item.get('type')}), ID: {parsed_item.get('id')}, Slot: {parsed_item.get('slot')}")
    ...
    for slot_key, original_item in original_accessories_data.items():
        ...
        if original_item.get('!color_mark') == '03 00 00 00':
            color_properties = [
                'main_color', 'main_shine', 'main_strength', 'main_texture',
                'sub_color', 'sub_shine', 'sub_strength', 'sub_texture'
            ]
            for prop_key in color_properties:
                if prop_key in original_item: # Only copy if it was present in the original data
