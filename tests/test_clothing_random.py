...
# 導入服裝數據
from game_data.clothing_data import CLOTHING_ITEMS, get_option_flags, is_colorful
...
# test utils
from tests.utils import _random_color_str, _random_hex_string, compare_dicts
...
def format_for_test(slot: str, item: dict = None) -> dict:
    ...
    slot_idx = slot_to_idx.get(
        slot, 0
    )  # Default to 0 if slot not found, though all should be in dict
...
def test_random_clothing_roundtrip(_):
    ...
    for slot_name in CLOTHING_ITEMS.keys():
        ...
        if slot_name == "swimsuit":
            ...
            if item_data:
                ...
                print(
                    f"Skipping swimsuit due to no available items or formatting issue."
                )
        ...
        if item_data:  # format_for_test might return None if no items are available
            ...
            print(
                f"Skipping slot '{slot_name}' due to no available items or formatting issue."
            )

    # Determine clothing_set (usually '通常' or '水著')
    # If a swimsuit is selected, set to '水著', otherwise '通常'
    clothing_set = (
        "水著" if original_clothing_data.get("swimsuit", {}).get("id") != 0 else "通常"
    )
    ...
    for slot_name in CLOTHING_ITEMS.keys():
        if slot_name in original_clothing_data and original_clothing_data[slot_name]:
            ...
            parsed_name = parsed_clothing_data.get(slot_name, {}).get(
                "#name"
            )  # Parser adds this
            ...
            if slot_name == "swimsuit":
                ...
                orig_opt_bottom = original_clothing_data["swimsuit"].get(
                    "option_bottom"
                )
                parsed_opt_bottom = parsed_clothing_data["swimsuit"].get(
                    "option_bottom"
                )
    ...
    for slot_name, original_item in original_clothing_data.items():
        if slot_name == "clothing_set":
            filtered_parsed_clothing_data[slot_name] = parsed_clothing_data.get(
                slot_name
            )
            continue
    ...
    if (
        "swimsuit" in original_clothing_data
        and "option_top" in original_clothing_data["swimsuit"]
    ):
        filtered_parsed_clothing_data["swimsuit"]["option_top"] = parsed_clothing_data[
            "swimsuit"
        ].get("option_top")
    if (
        "swimsuit" in original_clothing_data
        and "option_bottom" in original_clothing_data["swimsuit"]
    ):
        filtered_parsed_clothing_data["swimsuit"]["option_bottom"] = (
            parsed_clothing_data["swimsuit"].get("option_bottom")
        )
