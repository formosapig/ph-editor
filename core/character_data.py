...
class CharacterData:
    ...
    def _parse_all_sections(self):
        ...
        try:
            ...
            self.parsed_data["fixed_header"] = fixed_header_parser.parse_fixed_header(
                self.data_stream, debug_mode
            )

            # 2. 解析髮型數據
            # print(f"  [偏移: {self.data_stream.tell()}] 解析髮型數據...")
            self.parsed_data["hair"] = hair_parser.parse_hair_data(
                self.data_stream, debug_mode
            )

            # 3. 解析臉部數據
            # print(f"  [偏移: {self.data_stream.tell()}] 解析臉部數據...")
            self.parsed_data["face"] = face_parser.parse_face_data(
                self.data_stream, debug_mode
            )

            # 4. 解析身體數據
            # print(f"  [偏移: {self.data_stream.tell()}] 解析身體數據...")
            self.parsed_data["body"] = body_parser.parse_body_data(
                self.data_stream, debug_mode
            )

            # 5. 解析衣服數據
            # print(f"  [偏移: {self.data_stream.tell()}] 解析衣服數據...")
            self.parsed_data["clothing"] = clothing_parser.parse_clothing_data(
                self.data_stream, debug_mode
            )

            # 6. 解析配飾數據
            # print(f"  [偏移: {self.data_stream.tell()}] 解析配飾數據...")
            self.parsed_data["accessory"] = accessory_parser.parse_accessories_data(
                self.data_stream, debug_mode
            )
