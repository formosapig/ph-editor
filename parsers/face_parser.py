...
def parse_face_data(stream: BytesIO, debug_mode: bool = False) -> dict:
    ...
    try:
        # -- Overall Face 全體 2 --
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Overall Face' data.")
        ...
        # -- Eyebrows 眉毛 2--
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Eyebrows' data.")
        ...
        temp_eyebrows = {
            ...
            "!shine": _read_and_format_color(
                stream
            ),  # 可能有一個光澤顏色 (RGBA) 但不能改 似乎是 (205,205,205,255)
        ...
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Eyeballs (Left Eye)' data.")
        pupil_id = _read_int32(stream)
        temp_left_eyeball = {
            "pupil_id": pupil_id,  # 左瞳 ID
            "#name": get_face_by_id("eyeball", pupil_id),
            "sclera_color": _read_and_format_color(
                stream
            ),  # 左眼白顏色 (RGBA, alpha 無效)
            "pupil_color": _read_and_format_color(
                stream
            ),  # 左瞳孔顏色 (RGBA, alpha 無效)
        ...
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Eyeballs (Right Eye)' data.")
        pupil_id = _read_int32(stream)
        temp_right_eyeball = {
        ...
        # -- Overall Face 全體 1 --
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Face Dimensions' data.")
        face_data["overall"] = {
        ...
        # -- Chin 下巴 --
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Chin' data.")
        face_data["chin"] = {
        ...
        # -- Cheeks 頰 --
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Cheeks' data.")
        face_data["cheeks"] = {
            "lower_height": _read_and_format_to_value(stream),  # 下部上下
        ...
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Brow' data.")
        face_data["eyebrows"] = {
            "height": _read_and_format_to_value(stream),  # 上下
        ...
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Eyes' data.")
        face_data["eyes"] = {
            "height": _read_and_format_to_value(stream),  # 上下
            "horizontal_position": _read_and_format_to_value(stream),  # 橫位置
            "depth": _read_and_format_to_value(stream),  # 前後
        ...
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] Parsing 'Eyeballs (Basic Settings)' data."
            )
        face_data["eyeballs"] = {
        ...
        # -- Nose 鼻 --
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Nose' data.")
        face_data["nose"] = {
        ...
        # -- Mouth 口 --
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Mouth' data.")
        face_data["mouth"] = {
            "height": _read_and_format_to_value(stream),  # 上下
            "width": _read_and_format_to_value(stream),  # 橫幅
            "vertical_width": _read_and_format_to_value(stream),  # 縱幅
        ...
        # -- Ear 耳 --
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Ear' data.")
        face_data["ears"] = {
        ...
        # -- Eyelashes 睫毛 --
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Eyelashes' data.")
        ...
        face_data["eyelashes"] = {
            ...
            "!shine": _read_and_format_color(
                stream
            ),  # 睫毛光澤顏色 (RGBA) (185, 188, 177, 255) ??? 這個值會一直跳來跳去..
            "strength": _read_and_format_to_value(stream),  # 光澤強度
            "texture": _read_and_format_to_value(stream),  # 光澤質感
        }
        ...
        # -- Eyeshadow 眼影 --
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Eyeshadow' data.")
        ...
        # -- Blush 腮紅 --
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Blush' data.")
        ...
        # -- Lipstick 唇膏 --
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Lipstick' data.")
        ...
        # -- Mole 痣 --
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Mole' data.")
        ...
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] Parsing 'Eyeballs (Highlight)' data.")
