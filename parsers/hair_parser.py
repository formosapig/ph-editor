...
def _parse_single_hair_part(
    stream: BytesIO, part_name: str, debug_mode: bool = False
) -> dict:
    """
    ...
    try:
        ...
        if debug_mode:
            print(
                f"    [ERROR] Stream ended prematurely when parsing {part_name} data: {e}"
            )
        raise
    except Exception as e:
        if debug_mode:
            print(
                f"    [ERROR] Unknown error occurred when parsing {part_name} data: {e}"
            )
        raise
...
def parse_hair_data(stream: BytesIO, debug_mode: bool = False) -> dict:
    ...
    try:
        ...
        all_hair_data["front_hair"] = _parse_single_hair_part(
            stream, "front", debug_mode
        )
        ...
        if debug_mode:
            print(
                f"  [ERROR] Stream ended prematurely when parsing total hair data: {e}"
            )
        raise
    except Exception as e:
        if debug_mode:
    ...
    if debug_mode:
        print(
            f"  All hair data parts parsing complete. Next read position: {stream.tell()}"
        )
        try:
