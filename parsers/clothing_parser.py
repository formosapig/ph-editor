...
def parse_clothing_data(stream: BytesIO, debug_mode: bool = False) -> dict:
    ...
    Returns:
        一個字典，包含解析後的服裝數據。
    """
    clothing_data = {"clothing_set": None}
    ...
    try:
        ...
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] End of Top data. Expected padding at 0x0516."
            )
        ...
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] End of Bottom data. Expected padding at 0x0572."
            )
        ...
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] End of Bra data. Expected padding at 0x05CE."
            )
        ...
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] End of Panty data. Expected padding at 0x062A."
            )
        ...
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] End of Swimsuit (Full) data. Expected padding at 0x0686."
            )

        # 泳裝 - 上衣 (swimsuit_top)
        clothing_data["swimsuit_top"] = parse_clothing_item(
            stream, "swimsuit_top", debug_mode
        )
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] End of Swimsuit Top 1 data. Expected padding at 0x06E2."
            )

        # 泳裝 - 上衣 (swimsuit_bottom)
        clothing_data["swimsuit_bottom"] = parse_clothing_item(
            stream, "swimsuit_bottom", debug_mode
        )
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] End of Swimsuit Top 2 data. Expected padding at 0x073E."
            )
        ...
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] End of Gloves data. Expected padding at 0x079A."
            )

        # 褲襪 (pantyhose)
        clothing_data["pantyhose"] = parse_clothing_item(
            stream, "pantyhose", debug_mode
        )
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] End of Pantyhose data. Expected padding at 0x07F6."
            )
        ...
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] End of Socks data. Expected padding at 0x0852."
            )

        # 鞋子 (shoes)
        clothing_data["shoes"] = parse_clothing_item(stream, "shoes", debug_mode)
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] End of Shoes data.")
        ...
        if "option_bottom" in clothing_data["swimsuit"]:
            clothing_data["swimsuit"]["option_bottom"] = (
                "on" if option_bottom == 1 else "off"
            )

    except EOFError as e:
        if debug_mode:
            print(
                f"    [ERROR] Stream ended prematurely when parsing clothing data: {e}"
            )
        raise
