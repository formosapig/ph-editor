...
def serialize_accessory_item(
    accessory_item_data: dict, stream: BytesIO, debug_mode: bool = False
):
    """
    ...
    if color_mark == "03 00 00 00":
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] Serializing colorful accessory properties."
            )
        stream.write(
            _pack_color(accessory_item_data.get("main_color", "(255, 255, 255, 255)"))
        )
        stream.write(
            _pack_color(accessory_item_data.get("main_shine", "(255, 255, 255, 255)"))
        )
        ...
        stream.write(
            _pack_color(accessory_item_data.get("sub_color", "(255, 255, 255, 255)"))
        )
        stream.write(
            _pack_color(accessory_item_data.get("sub_shine", "(255, 255, 255, 255)"))
        )
        ...
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] No colorful accessory properties to serialize (color_mark: {color_mark})."
            )
...
def serialize_accessories_data(
    accessories_data: dict, stream: BytesIO, debug_mode: bool = False
):
    """
    ...
    try:
        ...
        for i in range(1, 11):  # Loop from 1 to 10
            ...
            if accessory_key in accessories_data:
                serialize_accessory_item(
                    accessories_data[accessory_key], stream, debug_mode
                )
            else:
                # If an accessory key is missing, write default (empty) accessory data
                if debug_mode:
                    print(
                        f"    [WARNING] Missing accessory data for {accessory_key}. Writing default empty values."
                    )
                serialize_accessory_item(
                    {}, stream, debug_mode
                )  # Pass an empty dict to write defaults

    except Exception as e:
        if debug_mode:
            print(
                f"  [ERROR] An error occurred during accessories data serialization: {e}"
            )
        raise
    finally:
        if debug_mode:
            print(
                f"  Accessories data serialization complete. Final stream position: {stream.tell()}"
            )
