...
def serialize_clothing_item(
    stream: BytesIO, item_data: dict, slot: str, debug_mode: bool = False
) -> None:
    """
    ...
    if debug_mode:
        print(
            f"    [Offset: {stream.tell()}] Serializing '{slot}' data (ID: {item_id})."
        )
...
def serialize_clothing_data(
    clothing_data: dict, stream: BytesIO, debug_mode: bool = False
):
    """
    ...
    try:
        ...
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] End of Top data serialization.")

        serialize_clothing_item(
            stream, clothing_data.get("bottom", {}), "bottom", debug_mode
        )
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] End of Bottom data serialization.")

        serialize_clothing_item(stream, clothing_data.get("bra", {}), "bra", debug_mode)
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] End of Bra data serialization.")

        serialize_clothing_item(
            stream, clothing_data.get("panty", {}), "panty", debug_mode
        )
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] End of Panty data serialization.")

        serialize_clothing_item(
            stream, clothing_data.get("swimsuit", {}), "swimsuit", debug_mode
        )
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] End of Swimsuit (Full) data serialization."
            )

        serialize_clothing_item(
            stream, clothing_data.get("swimsuit_top", {}), "swimsuit_top", debug_mode
        )
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] End of Swimsuit Top data serialization."
            )

        serialize_clothing_item(
            stream,
            clothing_data.get("swimsuit_bottom", {}),
            "swimsuit_bottom",
            debug_mode,
        )
        if debug_mode:
            print(
                f"    [Offset: {stream.tell()}] End of Swimsuit Bottom data serialization."
            )

        serialize_clothing_item(
            stream, clothing_data.get("gloves", {}), "gloves", debug_mode
        )
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] End of Gloves data serialization.")

        serialize_clothing_item(
            stream, clothing_data.get("pantyhose", {}), "pantyhose", debug_mode
        )
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] End of Pantyhose data serialization.")

        serialize_clothing_item(
            stream, clothing_data.get("socks", {}), "socks", debug_mode
        )
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] End of Socks data serialization.")

        serialize_clothing_item(
            stream, clothing_data.get("shoes", {}), "shoes", debug_mode
        )
        if debug_mode:
            print(f"    [Offset: {stream.tell()}] End of Shoes data serialization.")
        ...
        option_top_val = (
            1 if clothing_data.get("swimsuit", {}).get("option_top") == "on" else 0
        )
        option_bottom_val = (
            1 if clothing_data.get("swimsuit", {}).get("option_bottom") == "on" else 0
        )
    ...
    if debug_mode:
        print(
            f"  Clothing data serialization complete. Current stream position: {stream.tell()}"
        )
