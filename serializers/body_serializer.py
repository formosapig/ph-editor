...
def serialize_body_data(body_data: dict, stream: BytesIO):
    ...
    try:
        ...
        if 'overall' in body_data:
            ...
            stream.write(_parse_and_pack_float(overall.get('hue', 0), min_val=-50, max_val=50))
            stream.write(_parse_and_pack_float(overall.get('saturation', 0), scale=50)) # 預設 min_val=0, max_val=100
            stream.write(_parse_and_pack_float(overall.get('value', 0), scale=50)) # 預設 min_val=0, max_val=100
            stream.write(_parse_and_pack_float(overall.get('!alpha', 0))) # 預設 scale=100, min_val=0, max_val=100
            stream.write(_parse_and_pack_float(overall.get('gloss_strength', 0), scale=250)) # 預設 min_val=0, max_val=100
            stream.write(_parse_and_pack_float(overall.get('gloss_texture', 0), scale=125)) # 預設 min_val=0, max_val=100
            # !extra_value2 是一個 hex 字串
            stream.write(_pack_hex_to_bytes(overall.get('!extra_value2', "00 00 00 00"), 4))
            # flesh_strength
            stream.write(_parse_and_pack_float(overall.get('flesh_strength', 0))) # 預設 scale=100, min_val=0, max_val=100
        ...
        if 'tattoo' in body_data:
            ...
            stream.write(_pack_hex_to_bytes(tat_data.get('!padding1', "00 00 00 00"), 4))
        ...
        if 'breast' in body_data:
            breast_fields = [
                'size', 'vertical_position', 'horizontal_spread',
                'horizontal_position', 'angle', 'firmness',
                'areola_prominence', 'nipple_thickness'
            ]
        ...
        if 'overall' in body_data:
            stream.write(_parse_and_pack_float(body_data['overall'].get('head_size', 0)))

        # -- Upper Body 上半身 --
        if 'upper_body' in body_data:
            upper_body_fields = [
                'neck_width', 'neck_thickness', 'torso_shoulder_width', 'torso_shoulder_thickness',
                'torso_upper_width', 'torso_upper_thickness', 'torso_lower_width', 'torso_lower_thickness'
            ]
            for field in upper_body_fields:
                stream.write(_parse_and_pack_float(body_data['upper_body'].get(field, 0)))

        # -- Lower Body 下半身 --
        if 'lower_body' in body_data:
            lower_body_fields = [
                'waist_position', 'waist_upper_width', 'waist_upper_thickness',
                'waist_lower_width', 'waist_lower_thickness', 'hip_size', 'hip_angle'
            ]
            for field in lower_body_fields:
                stream.write(_parse_and_pack_float(body_data['lower_body'].get(field, 0)))
        ...
        if 'breast' in body_data: # 屬於 breast 的子項
            stream.write(_parse_and_pack_float(body_data['breast'].get('nipple_erectness', 0)))
        ...
        if 'breast' in body_data and 'nipples' in body_data['breast']:
            ...
            stream.write(_parse_and_pack_float(nip_data.get('hue', 0), scale=100, min_val=-50, max_val=50))
            ...
            stream.write(_parse_and_pack_float(nip_data.get('gloss_strength', 0), scale=250))
            stream.write(_parse_and_pack_float(nip_data.get('gloss_texture', 0), scale=125))

        # -- Tan Lines 曬痕 --
        if 'tan_lines' in body_data:
            tan_data = body_data['tan_lines']
            stream.write(_pack_int32(tan_data.get('id', 0)))
            stream.write(_parse_and_pack_float(tan_data.get('hue', 0), scale=100, min_val=-50, max_val=50))
            ...
            stream.write(_pack_hex_to_bytes(tan_data.get('!padding1', "00 00 00 00"), 4))

        # -- Nails 爪 --
        if 'nails' in body_data:
            nails_data = body_data['nails']
            stream.write(_parse_and_pack_float(nails_data.get('hue', 0), scale=100, min_val=-50, max_val=50))
            stream.write(_parse_and_pack_float(nails_data.get('saturation', 0), scale=50))
            ...
            stream.write(_pack_hex_to_bytes(nails_data.get('!padding1', "00 00 00 00"), 4))

            # -- Nail Polish 爪 指甲油 --
            if 'polish' in nails_data: # 屬於 nails 的子項
                polish_data = nails_data['polish']
                stream.write(_pack_color(polish_data.get('color', "(255, 255, 255, 255)")))
                stream.write(_pack_color(polish_data.get('!shine', "(255, 255, 255, 255)")))
                stream.write(_parse_and_pack_float(polish_data.get('shine_strength', 0)))
        ...
        if 'breast' in body_data and 'nipples' in body_data['breast']:
            stream.write(_parse_and_pack_float(body_data['breast']['nipples'].get('areola_size', 0)))
